...
import springboot.fileConvert.utils.LocalOfficeUtils;
...
public class OfficePluginManager {
    ...
    public void startOfficeManager() throws OfficeException {
        if (OSUtils.IS_OS_WINDOWS || OSUtils.IS_OS_MAC) {
            logger.info("windows、mac环境跳过office组件初始化...");
            return;
        }
        ...
        boolean killOffice = killProcess(ports);
        if (killOffice) {
     ...
     * @return
     */
    private boolean killProcess(int[] ports) {
        boolean flag = false;
        try {
            for (int port : ports) {
                String findCmd = String.format(
                        "ps -ef | grep soffice.bin | grep 'port=%d' | grep -v grep | awk '{print $2}'",
                        port
                );
                Process findProcess = Runtime.getRuntime().exec(new String[]{"sh", "-c", findCmd});
                ByteArrayOutputStream baos = new ByteArrayOutputStream();
                InputStream inputStream = findProcess.getInputStream();
                byte[] buffer = new byte[256];
                int bytesRead;
                while ((bytesRead = inputStream.read(buffer)) > 0) {
                    baos.write(buffer, 0, bytesRead);
                }
                String pidList = baos.toString().trim();
                if (!pidList.isEmpty()) {
                    String[] pids = pidList.split("\\s+");
                    for (String pid : pids) {
